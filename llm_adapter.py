...
class OpenAICompatiblePlanner:
    ...
    PATCH_FIELDS = {
        "productType", "purpose", "quantity", "quantityValue", "quantityUnit", "size", "pages", "orientation", "paper",
    ...
    def plan(self, text: str, order: dict[str, Any], tools: list[dict[str, Any]],
        ...
        if not self.enabled:
            return None
        prompt = {"role": "system", "content": "你是印刷订单助手，使用简洁自然的中文和非专业用户对话。每次只输出一个 JSON 对象，不要 Markdown 代码围栏：{reply:string,patch:object,tool:{name:string,arguments:object}|null}。reply 是给用户看的自然语言，必须在需要时提出下一步问题；patch 只能使用订单字段 productType,purpose,quantity,quantityValue,quantityUnit,size,pages,orientation,paper,printing,finishing,binding,deadline,budget,platform,productSpecs。quantity 是给用户看的数量文本，quantityValue 是数字，quantityUnit 是张、份、个等单位；三者不一致时只提交 quantity，让系统统一规范化。品类专属参数必须放在 productSpecs 对象中（例如 folding、paperParts、boxSize、boxStructure、labelMaterial、labelShape、bagSize、handle、cupVolume、displayMaterial、install、boardThickness）。工具只能从提供的白名单中选择，只有信息足够或用户明确要求时调用工具。收到 toolResult 时，优先解释工具结果并给出下一步，不要重复调用同一个工具。不要编造价格、供应商能力或已提交订单。"}
        messages = [prompt]
